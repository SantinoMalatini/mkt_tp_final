...
def build_fact_web_sessions(data: Dict[str, pd.DataFrame], 
    ...
    fact_sessions['start_date'] = pd.to_datetime(fact_sessions['started_at']).dt.date
    fact_sessions = fact_sessions.merge(
        date_map.rename(columns={'date_id': 'session_start_date_id', 'full_date': 'start_date'}),
        on='start_date',
        how='left'
    )

    fact_sessions['end_date'] = pd.to_datetime(fact_sessions['ended_at']).dt.date
    fact_sessions = fact_sessions.merge(
        date_map.rename(columns={'date_id': 'session_ended_id', 'full_date': 'end_date'}),
        on='end_date',
        how='left'
    )
    
    fact_sessions.drop(columns=['start_date', 'end_date'], inplace=True)
    ...
    # Agregar dimensiones degeneradas si existen
    ...
    # === Limpieza de nulos ===
    fk_cols = ['session_start_date_id', 'session_ended_id', 'customer_id']
    fact_sessions[fk_cols] = fact_sessions[fk_cols].fillna(0).astype(int)
